        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th>Monto Neto (A recibir)</th>
                    <th>Estado de Pago Vendedor</th>
                </tr>
            </thead>
            <tbody>
                <% if(ventas != null && !ventas.isEmpty()) { 
                    for(VentaAdminDTO v : ventas) {
                        // Bug fix: usar estadoPagoVendedor (campo de pagos_vendedor) y valor 'PAGADO'
                        boolean esPagado = "PAGADO".equals(v.getEstadoPagoVendedor());
                %>
                <tr class="<%= esPagado ? "" : "hide-on-print" %>">
                    <td><%= v.getFechaOrden() %></td>
                    <td><%= v.getNombreProducto() %></td>
                    <td>$<%= v.getSubtotalLinea() %></td>
                    <td>$<%= v.getMontoNeto() %></td>
                    <td>
                        <span class="badge <%= esPagado ? "badge-success" : "badge-warning" %>">
                            <%= v.getEstadoPagoVendedor() != null ? v.getEstadoPagoVendedor() : "PENDIENTE" %>
                        </span>
                    </td>
                </tr>
                <%  }
